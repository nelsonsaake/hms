...
<body class="bg-gray-50 text-gray-900 font-instrument-sans">
  ...
  <!-- Hero -->
  <section class="relative h-[70vh] bg-cover bg-center" style="background-image: {{ $roomImages[0] }}">
    ...
    <div class="relative z-10 container mx-auto h-full flex flex-col justify-center items-center text-center px-4">
      ...
      <p class="text-lg md:text-xl text-gray-100 mb-8 max-w-xl">
        Experience modern comfort and unforgettable stays in every room at Blue Top Hotel. ajajajd {{ typeOf($roomImages) }}
      </p>
      <a href="#rooms" class="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-full font-medium transition">
        View Rooms
      </a>
    </div>
  </section>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        @forelse ($rooms as $room)
          <div class="rounded-xl overflow-hidden shadow-md hover:shadow-xl transition bg-white">
            <img loading="lazy" src="{{ $room->roomImages?->first()?->url ?? '/images/default-room.jpg' }}" alt="{{ $room->type }}" class="w-full h-56 object-cover">
            <div class="p-6">
