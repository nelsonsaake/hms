    ...
    <div class="flex w-full flex-1 flex-col gap-4 rounded-xl mt-8">
        ...
        <form wire:submit.prevent="save" action="{{ route('bookings.store') }}" method="POST" class="space-y-6">
            ...
            <flux:select wire:model="room_id" name="room_id" id="room_id" :label="__('Room')"
                ...
                @foreach ($rooms as $v)
                    <option value="{{ $v->id }}" {{ old('room_id') == $v->id ? 'selected' : '' }}>
                        {{ $v->name ?? idfmt($v->id) }}
                    </option>
                @endforeach
            </flux:select>

            <flux:input :label="__('Check In')" wire:model="check_in" id="check_in" name="check_in" type="date"
                required value="{{ old('check_in', hdfmt(now())) }}" :error="$errors->has('check_in')"
                :error-message="$errors->first('check_in')" />

            <flux:input :label="__('Check Out')" wire:model="check_out" id="check_out" name="check_out" type="date"
                required value="{{ old('check_out', hdfmt(now())) }}" :error="$errors->has('check_out')"
            ...
            <flux:select wire:model="status" name="status" id="status" :label="__('Status')"
                ...
                @foreach (BookingStatus::values() as $v)
                    <option value="{{ $v }}" {{ old('status') == $v ? 'selected' : '' }}>
                        {{ efmt($v) }}
                    </option>
                @endforeach
            </flux:select>

            <flux:input :label="__('Guest Name')" wire:model="guest_name" id="guest_name" name="guest_name"
                type="text" required value="{{ old('guest_name') }}" :error="$errors->has('guest_name')"
                :error-message="$errors->first('guest_name')" />

            <flux:input :label="__('Guest Email')" wire:model="guest_email" id="guest_email" name="guest_email"
                type="text" required value="{{ old('guest_email') }}" :error="$errors->has('guest_email')"
                :error-message="$errors->first('guest_email')" />

            <flux:input :label="__('Guest Phone')" wire:model="guest_phone" id="guest_phone" name="guest_phone"
                type="number" required value="{{ old('guest_phone') }}" :error="$errors->has('guest_phone')"
                :error-message="$errors->first('guest_phone')" />

            <flux:input :label="__('From Date')" wire:model="from_date" id="from_date" name="from_date" type="date"
                required value="{{ old('from_date', hdfmt(now())) }}" :error="$errors->has('from_date')"
                :error-message="$errors->first('from_date')" />

            <flux:input :label="__('To Date')" wire:model="to_date" id="to_date" name="to_date" type="date"
                required value="{{ old('to_date', hdfmt(now())) }}" :error="$errors->has('to_date')"
