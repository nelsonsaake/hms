    ...
    <div class="flex w-full flex-1 flex-col gap-4 rounded-xl mt-8">
        ...
        <form wire:submit.prevent="save" action="{{ route('reservations.store') }}" method="POST" class="space-y-6">
            ...
            <flux:select wire:model="room_id" name="room_id" id="room_id" :label="__('Room')"
                ...
                @foreach ($rooms as $v)
                    <option value="{{ $v->id }}" {{ $v->id == $roomId ? 'selected' : '' }}>
            ...
            <flux:input :label="__('Guest Name')" wire:model="guest_name" id="guest_name" name="guest_name"
                type="text" required value="{{ old('guest_name', authUser()->name) }}"
                :error="$errors->has('guest_name')" :error-message="$errors->first('guest_name')" />

            <flux:input :label="__('Guest Email')" wire:model="guest_email" id="guest_email" name="guest_email"
                type="text" required value="{{ old('guest_email', authUser()->email) }}"
                :error="$errors->has('guest_email')" :error-message="$errors->first('guest_email')" />
            ...
            <flux:input :label="__('From Date (Optional)')" wire:model="from_date" id="from_date" name="from_date"
                type="date" value="{{ old('from_date') }}" :error="$errors->has('from_date')"
                :error-message="$errors->first('from_date')" />

            <flux:input :label="__('To Date (Optional)')" wire:model="to_date" id="to_date" name="to_date"
                type="date" value="{{ old('to_date') }}" :error="$errors->has('to_date')"
